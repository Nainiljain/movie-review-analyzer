...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // SPEECH-TO-TEXT
  // ==========================
  const startSpeechToText = (inputField) => {
    ...
    recog.lang = 'en-US';
    recog.start();
    recog.onresult = (event) => { inputField.value += event.results[0][0].transcript; };
    recog.onerror = (e) => console.error("Speech recognition error:", e.error);
  };
  ...
  if (isMobileOrTablet() && ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window)) {
    ...
    hotwordRecog.onerror = (e) => console.error("Hotword error:", e.error);
  ...
  const displayMovies = (movies) => {
    ...
    if (!movies || movies.length === 0) return movieContainer.innerHTML = "<p>No movies found.</p>";
  ...
  const fetchMovies = (query="") => {
    ...
    if (!query) fetch("/search_tmdb").then(r=>r.json()).then(displayMovies).catch(console.error);
    else if (knownGenres.includes(lower)) fetch(`/filter_movies?genre=${encodeURIComponent(lower)}`).then(r=>r.json()).then(displayMovies).catch(console.error);
    else fetch(`/search_tmdb?q=${encodeURIComponent(query)}`).then(r=>r.json()).then(displayMovies).catch(console.error);
  };

  document.getElementById("btn-search").addEventListener("click", () => fetchMovies(searchInput.value.trim()));
  ...
  const displayReviews = (reviews) => {
    ...
    if (!reviews || reviews.length === 0) return reviewContainer.innerHTML = "<p>No reviews yet.</p>";
    reviews.forEach(r => {
  ...
  const loadReviews = () => fetch("/filter_reviews").then(r => r.json()).then(displayReviews).catch(console.error);
  ...
  document.getElementById("btn-add-review").addEventListener("click", () => {
    ...
    fetch("/add_review", {
      ...
      .then(() => { loadReviews(); reviewInput.value = ""; document.getElementById("review-movie-title").value = ""; })
      .catch(console.error);
  });
